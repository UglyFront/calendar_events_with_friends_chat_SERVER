...
let UserServices = class UserServices {
    ...
    async updatePassword(dto) {
        ...
        try {
            ...
            console.log(e);
            throw new common_1.BadRequestException("Ошибка почты...");
        }
    }
    async checkCode(dto) {
        const user = await this.userDB.find({ where: {
        ...
        if (user[0].code === dto.code) {
            ...
            throw new common_1.BadRequestException("Код не верен, новый код отправлен на почту!");
        }
    }
};
...
//# sourceMappingURL=user.servicves.js.map